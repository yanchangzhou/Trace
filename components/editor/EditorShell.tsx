...
export default function EditorShell() {
  ...
  const openAIInlineRef = useRef(openAIInline);
  openAIInlineRef.current = openAIInline;
  ...
  useEffect(() => {
    ...
    return () => setAIInlineHandler(null);
  }, []);

  // Register insert/replace handlers for AI inline popup
  useEffect(() => {
    if (!editor) return;
    registerInsertGeneratedText((text: string) => {
      editor.chain().focus().insertContent(text).run();
    });
    registerReplaceSelection((text: string) => {
      editor.chain().focus().deleteSelection().insertContent(text).run();
    });
    return () => {
      registerInsertGeneratedText(null);
      registerReplaceSelection(null);
    };
