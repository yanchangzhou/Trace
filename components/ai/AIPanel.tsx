...
import type { AIMessage, AIRequest, WritingStyle, StyleProfile } from '@/types';
import StreamingComposer from './StreamingComposer';
import { copyToClipboard } from '@/lib/clipboard';
import { generateAIStream } from '@/lib/tauri';
import type { ChatMessage, FileInfo } from '@/lib/tauri';
...
export default function AIPanel() {
  ...
  const handleSend = useCallback(async () => {
    ...
    // Build AI request for context from selected files
    const aiRequest: AIRequest = {
      action: activeAction as AIRequest['action'],
      context_file_ids: currentFiles.map((f) => f.id),
      style: selectedStyle,
      prompt: text,
    };

    const fileInfos: FileInfo[] = currentFiles.map((f) => ({
      name: f.name,
  ...
  }, [inputText, isStreaming, messages, activeAction, selectedStyle, currentFiles]);
